...
namespace eWolfRoadBuilder
{
    ...
    public class MeshBuilderSection
    {
        ...
        public void Build(RoadCrossSection rA, RoadCrossSection rB, float texturePercent)
        {
            ...
            float percentageRoadMain = GetMainRoadLength(rA, rB);
            flip = AddMainRoadSection(rA, rB, triangles, percentageRoadMain);

            if (!RoadConstructorHelper.CrossSectionDetails.WithCurbValue)
                return;

            float percentageTopLeft = GetCurbTopLeftLength(rA, rB);
            AddCurbTopLeft(rA, rB, flip, triangles, percentageTopLeft);

            float percentageTopRight = GetCurbTopRightLength(rA, rB);
            AddCurbTopRight(rA, rB, flip, triangles, percentageTopRight);

            if (RoadConstructorHelper.CrossSectionDetails.HasCurbDataValue)
            {
                AddCurbUpLeft(rA, rB, flip, triangles, percentageTopLeft);
                AddCurbUpRight(rA, rB, flip, triangles, percentageTopRight);
            }

            // Add the Curb Down polys.
            AddCurbLeftDrop(rA, rB, flip, triangles, percentageTopLeft);
            AddCurbRightDrop(rA, rB, flip, triangles, percentageTopRight);

            if (RoadConstructorHelper.CrossSectionDetails.UnderRoad)
            {
                List<int> underSideTriangles = _roadObject.GetTriangles(MaterialFrequency.UnderRoadMaterial);
                AddUnderSide(rA, rB, !flip, underSideTriangles, percentageTopLeft);
            }
        }

        private void AddUnderSide(RoadCrossSection rA, RoadCrossSection rB, bool flip, List<int> triangles, float percentageTopLeft)
        {
            ...
            _roadObject.MeshUVs.Add(new Vector2(1, 0));
            _roadObject.MeshUVs.Add(new Vector2(0, 0));
            _roadObject.MeshUVs.Add(new Vector2(1, 1));
            _roadObject.MeshUVs.Add(new Vector2(0, 1));
        ...
        private void AddCurbLeftDrop(RoadCrossSection rA, RoadCrossSection rB, bool flip, List<int> triangles, float texturePercent)
        {
        ...
        /// <param name="triangles">The list of triangle to add to</param>
        /// <param name="texturePercent">The percentage of texture to use</param>
        private void AddCurbRightDrop(RoadCrossSection rA, RoadCrossSection rB, bool flip, List<int> triangles, float texturePercent)
        {
        ...
        /// <param name="triangles">The list of triangle to add to</param>
        /// <param name="texturePercent">The percentage of texture to use</param>
        private void AddCurbUpLeft(RoadCrossSection rA, RoadCrossSection rB, bool flip, List<int> triangles, float texturePercent)
        {
        ...
        /// <param name="triangles">The list of triangle to add to</param>
        /// <param name="texturePercent">The percentage of texture to use</param>
        private void AddCurbUpRight(RoadCrossSection rA, RoadCrossSection rB, bool flip, List<int> triangles, float texturePercent)
        {
        ...
        private float GetCurbTopLeftLength(RoadCrossSection rA, RoadCrossSection rB)
        {
            float roadWidth = (rA.Left - rA.Right).magnitude;

            float roadLength = (rA.CurbLeftLip - rB.CurbLeftLip).magnitude;
            float roadLengthb = (rA.CurbLeftEnd - rB.CurbLeftEnd).magnitude;

            float a = (roadLength + roadLengthb) / 2;
            if (a > roadWidth)
                a = roadWidth;
            float b = a / roadWidth;
        ...
        private float GetCurbTopRightLength(RoadCrossSection rA, RoadCrossSection rB)
        {
            float roadWidth = (rA.Left - rA.Right).magnitude;

            float roadLength = (rA.CurbRightLip - rB.CurbRightLip).magnitude;
            float roadLengthb = (rA.CurbRightEnd - rB.CurbRightEnd).magnitude;
        ...
        private float GetMainRoadLength(RoadCrossSection rA, RoadCrossSection rB)
        {
            float roadWidth = (rA.Left - rA.Right).magnitude;

            float roadLength = (rA.Middle - rB.Middle).magnitude;

            if (roadLength > roadWidth)
                roadLength = roadWidth;
            float b = roadLength / roadWidth;
