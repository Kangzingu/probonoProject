...
namespace eWolfRoadBuilder
{
    ...
    public class MeshBuilderJunction : MeshBuilderBase
    {
        ...
        private void DrawUnderRoad(JunctionArray ja)
        {
            if (!RoadConstructorHelper.CrossSectionDetails.WithCurbValue)
                return;

            if (!RoadConstructorHelper.CrossSectionDetails.HasUnderRoad)
                return;

            float drop = RoadConstructorHelper.CrossSectionDetails.CurbEdgeDropValue;
            Vector3 down = Vector3.up * -drop;

            DrawQuad(ja.LeftFarSide[6] + down, ja.RightFarSide[6] + down, ja.LeftSide[4] + down, ja.RightSide[4] + down,
                new Vector2(0, 0),
                new Vector2(0, 1),
                new Vector2(1, 0),
                new Vector2(1, 1), MaterialFrequency.UnderRoadMaterial);

            DrawQuad(ja.LeftFarSide[1] + down, ja.LeftFarSide[6] + down, ja.LeftSide[1] + down, ja.LeftSide[4] + down,
                new Vector2(0, 0),
                new Vector2(0, 1),
                new Vector2(1, 0),
                new Vector2(1, 1), MaterialFrequency.UnderRoadMaterial);

            DrawQuad(ja.RightSide[1] + down, ja.RightSide[4] + down, ja.RightFarSide[1] + down, ja.RightFarSide[6] + down,
                new Vector2(0, 0),
                new Vector2(0, 1),
                new Vector2(1, 0),
                new Vector2(1, 1), MaterialFrequency.UnderRoadMaterial);
