...
namespace eWolfRoadBuilder
{
    ...
    public class MeshBuilderFiveRoad : MeshBuilderBase
    {
        ...
        public void Build()
        {
            ...
            DrawUnderRoad(outterInner);
        }

        private void DrawUnderRoad(OutterInner oi)
        {
            if (!RoadConstructorHelper.CrossSectionDetails.WithCurbValue)
                return;
            ...
            for (int i = 0; i < _totalRoads; i++)
            {
                DrawTri(_roadNetworkNode.transform.position + down, oi.Outter[i].CurbRightEndDrop, oi.Outter[i].CurbLeftEndDrop,
                            new Vector2(0, 1),
                            new Vector2(1, 1),
                            new Vector2(1, 0), MaterialFrequency.UnderRoadMaterial);
            }
        }
